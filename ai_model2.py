...
def get_stock_order_ratio(amount: int, stocks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for i in range(len(stocks)):
        make_model(next((item[1] for item in open_dif_data_list if item[0] == stocks[i]['name']), None),
                   stocks[i]['name'])
...
def add_news_label(data: pd.DataFrame, name: str) -> pd.DataFrame:
    ...
    title_list, date_list = scrape_news_titles_and_dates(news_url_base, 5)

    for title, date in zip(title_list, date_list):
        if name in title:
            today_news_title.append(title)
            today_date_list.append(date)
...
def calculate_stock_amounts(predicted_results: Dict[str, Dict[str, float]], amount: float, stocks) -> List[
    Dict[str, Any]]:
    stock_orders = []
    stock_code = ''
...
def get_stocklist_top50(predicted_results: Dict[str, Dict[str, float]], stocks: List[Dict[str, Any]]) -> List[
    Dict[str, Any]]:
